...
class DifferentiableSparseHypergraph(nn.Module):
    ...
    def get_loss(self, target_density=0.5):
        ...
        Args:
            target_density (float): 期望保留的连接比例 (0.0 ~ 1.0).
                                    例如 0.5 表示希望至少剪掉 50% 的边。
        """
        ...
        # [核心优化 1 & 2] 目标导向的稀疏损失
        # 逻辑：只有当当前密度 > 目标密度时，才产生惩罚。
        # 一旦密度降到了 target_density 以下，loss 变为 0，停止“追杀”剩余的连接。
        # 这既防止了过度稀疏化，也实现了达到目标自动停止。
