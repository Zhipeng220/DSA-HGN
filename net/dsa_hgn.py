...
class DifferentiableSparseHypergraph(nn.Module):
    ...
    Fixed Version:
    1. Prevents Orthogonality Loss Explosion (using MSE instead of Fro norm).
    2. Prevents Sparsity Deadlock (using threshold clamping).
    """
    ...
    def __init__(self, in_channels, num_hyperedges, ratio=8, use_virtual_conn=True, **kwargs):
        ...
        self.last_h = None  # 用于保存关联矩阵，供 visualization 使用
    ...
    def forward(self, x):
        ...
        # [CRITICAL FIX 1] 限制阈值范围，防止 diff 永远小于 -1 导致梯度死锁
        # 即使模型想完全关闭连接，也不允许阈值跑得太远
        thresh = torch.clamp(thresh, min=-5.0, max=5.0)
    ...
    def get_loss(self):
        ...
        if not self.use_virtual_conn or self.last_mask is None:
            return torch.tensor(0.0, device=self.key_prototypes.device), torch.tensor(0.0,
