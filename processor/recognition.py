...
class FT_Processor(Processor):
    """
        Processor for Paper A Supervised Training (with L1 Regularization).
    """
    ...
    def test(self, epoch):
        ...
        for data, label in loader:
            ...
            # get loss
            # [FIXED BUG]: 'train' or 'test' is always True. Changed to check if phase is in list.
        ...
        # [FIXED BUG]: Logic consistency check
        if self.arg.phase in ['train', 'test']:
            self.label = np.concatenate(label_frag)

        # [FIXED BUG]: Changed key from 'mean_loss' to 'eval_mean_loss' to match processor.py
    ...
    def get_parser(add_help=False):
        ...
        # [FIX] 补充配置文件中存在的参数，防止 AssertionError
